...
`a * x ≡ 1 (mod m)` 을 만족하는 `x`를 `a`의 모듈러 역원이라고 해.
...
암호학적으로 안전한 난수가 필요하다면 반드시 **CSPRNG (Cryptographically Secure PRNG)**를 써야 해.
